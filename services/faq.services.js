...
const {getFeatureValue} = require("../config/helper")
...
const faq = async (brand, model, variant, cityId) => {
  ...
  try {
    ...
    const displacement = getFeatureValue(featureMap(Features.displacement));
    const typeOfEngine = getFeatureValue(featureMap(Features.typeOfEngine));
    const mileage = getFeatureValue(featureMap(Features.mileage));
    const airbags = getFeatureValue(featureMap(Features.noOfAirbags));
    const drivingRange = getFeatureValue(featureMap(Features.drivingRange));
    const batteryCapacity = getFeatureValue(featureMap(Features.batteryCapacity));
    const power = getFeatureValue(featureMap(Features.power));
    const evPower = getFeatureValue(featureMap(Features.evPower));
    const torque = getFeatureValue(featureMap(Features.torque));
    const groundClearance = getFeatureValue(featureMap(Features.groundClearance));
    const bootSpace = getFeatureValue(featureMap(Features.bootSpace));
    const fuelTankCapacity = getFeatureValue(featureMap(Features.fuelTankCapacity));
    const batteryType = getFeatureValue(featureMap(Features.batteryType));
    const sunroof = getFeatureValue(featureMap(Features.sunroof));
    const moonroof = getFeatureValue(featureMap(Features.moonroof));
    const climateControl = getFeatureValue(featureMap(Features.climateControl));
    const rearCamera = getFeatureValue(featureMap(Features.rearCamera));
    const rainSensingWipers = getFeatureValue(featureMap(Features.rainSensingWipers));
    const voiceCommands = getFeatureValue(featureMap(Features.voiceCommands));
    const cruiseControl = getFeatureValue(featureMap(Features.cruiseControl));
    const antiLockBrakingSystem = getFeatureValue(featureMap(Features.antiLockBrakingSystem));
    const blindSpotMonitoring = getFeatureValue(featureMap(Features.blindSpotMonitoring));
    const speedSensingAutoDoorLock = getFeatureValue(featureMap(Features.speedSensingAutoDoorLock));
    const hillDescentControl = getFeatureValue(featureMap(Features.hillDescentControl));
    const crashSensor = getFeatureValue(featureMap(Features.crashSensor));
    const tyrePressureMonitor = getFeatureValue(featureMap(Features.tyrePressureMonitor));
    const parkingSensors = getFeatureValue(featureMap(Features.parkingSensors));
    const highSpeedAlert = getFeatureValue(featureMap(Features.highSpeedAlert));
    const parkAssist = getFeatureValue(featureMap(Features.parkAssist));
    const hillAssist = getFeatureValue(featureMap(Features.hillAssist));
    const brakeAssist = getFeatureValue(featureMap(Features.brakeAssist));
    const sosAssistance = getFeatureValue(featureMap(Features.sosAssistance));
    const threeSixtyCamera = getFeatureValue(featureMap(Features.threeSixtyCamera));
    const laneWatchCamera = getFeatureValue(featureMap(Features.laneWatchCamera));
    const laneChangeIndicator = getFeatureValue(featureMap(Features.laneChangeIndicator));
    const appleCarPlay = getFeatureValue(featureMap(Features.appleCarPlay));
    const androidAuto = getFeatureValue(featureMap(Features.androidAuto));
    const wirelessPhoneCharger = getFeatureValue(featureMap(Features.wirelessPhoneCharger));
    const bluetoothConnectivity = getFeatureValue(featureMap(Features.bluetoothConnectivity));
    const topSpeed = getFeatureValue(featureMap(Features.topSpeed));
    const zeroToHundredKm = getFeatureValue(featureMap(Features.zeroToHundredKm));
