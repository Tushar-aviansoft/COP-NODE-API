...
exports.up = function (knex) {
  return knex.raw(`
    ...
    FROM
      ...
      JOIN cop_cl_data ON cop_cl_data.model_id = cop_fv.model_id
      JOIN cop_cl_ms ON cop_cl_data.cl_id = cop_cl_ms.cl_id
      JOIN base_variant_id mv ON mv.model_id = cop_fv.model_id 
      AND mv.variant_id = cop_fv.variant_id
    WHERE
      cop_features_ms.features_name IN ('Displacement', 'Range', 'Power (EV)', 'Power')
      AND cop_cl_ms.cl_name IN ('Popular Cars', 'EV Cars')
    GROUP BY 
      cop_fv.model_id, mv.variant_id
  `);
};
